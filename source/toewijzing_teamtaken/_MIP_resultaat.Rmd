...
Tabel \@ref(tab:{{id}}-slack) toont de waarde van de slack variabelen. Zoals reeds eerder beschreven, $s^-_j >0$ indien teamlid $j$ meer tijd spendeert aan teamtaken dan hij eigenlijk zou moeten. $s^+_j >0$ indien team $j$ minder tijd spendeert aan teamtaken dan hij eigenlijk zou moeten. `r ifelse(round(smin, 5) == round(splus, 5), "De som over alle $s^+_j >0$ is gelijk aan de som over alle $s^-_j >0$. Dit betekent dat er juist even veel teamleden zijn toegewezen aan iedere taak als nodig is (de eerste constraint in formulering \\ref{eq:ip-formulering} is *tight*).", "De som over alle rijen van de tweede kolom is niet gelijk aan de som over alle rijen van de eerste kolom. Dit wil zeggen dat er meer teamleden zijn toegewezen aan sommige taken dan strikt noodzakelijk. Dit fenomeen is mogelijk indien $p_t$ hoog gekozen wordt.")`
