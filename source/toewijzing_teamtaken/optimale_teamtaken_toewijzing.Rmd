...
gebruikte_top <- 15
gewicht_afwijking_tijdsinvestering <- 5 #per dag afwijking
...
Er zat een fout in de data: de taak "Comissie voor wetenschappelijke integriteit" moet bij Hans blijven; we voegen de beperking $x_ij \geq 1$ toe om dit op te leggen. 

```{r echo=FALSE, warning = FALSE, include = FALSE}
result <- model %>% 
  add_constraint(x[i, j] >= 1, i = 14, j = 3) %>%
  solve_model(with_ROI("glpk", verbose = TRUE))
id <- "alternatiefa"
```
...
Na nog enkele extra testen (kosten wijzigen, beperkingen toevoegen, doelfunctie aanpassen,...) is het duidelijke dat er een keuze gemaakt moet worden tussen een licht ongebalanceerde tijdsbesteding (beste lijkt eerste alternatief) of een grote taak aan iemand toewijzen die de taak niet in zijn/haar top 15 had gezet (Derde alternatief). Waar geven we de voorkeur aan? 
