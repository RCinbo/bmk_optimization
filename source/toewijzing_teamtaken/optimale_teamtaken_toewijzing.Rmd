...
gebruikte_top <- 15
gewicht_afwijking_tijdsinvestering <- 5
...
data_voorkeur2[16, which(is.na(data_voorkeur2[16, ]))] <- data_voorkeur[16, which(is.na(data_voorkeur2[16, ]))]
...
```{r taken, echo=FALSE}
data_taken %>%
  dplyr::select(-`uitgebreide_taakbeschrijving`, -taak_houden) %>%
  kable("latex",
        booktabs = T,
...
data_voorkeur %>% 
  ...
  kable(
    ...
    col.names = c("$w_{ij}$", data_tijd$teamlid),
    escape = FALSE) %>%
  kableExtra::kable_styling() %>%
...
In deze sectie beschrijven we de startoplossing. Deze oplossing is optimaal in het MIP model (formulering \@ref(eq:ip-formulering)). Hoewel de oplossing optimaal is, is het mogelijk dat er toch een onevenwicht is dat bijgesteld moet worden. De waardes van de verschillende kostenparameters zijn:
...
- Beloning om taken met een interactie van respectievelijk typetoe te wijzen aan dezelfde persoon: `r str_c(-1*(gewicht_interactie$kost), collapse = ", ")`
...
```{r echo=FALSE, results='asis', eval = params$anoniem}
cat(
  knitr::knit(text = knitr::knit_expand('_MIP_resultaat_anoniem.Rmd', envir = environment(), quiet = TRUE), quiet = TRUE
  ))
```

Aangezien er een grote afwijking is van een eerlijke tijdsverdeling, maken we de kost voor deze afwijking $p_t$ hoger. We hopen dat er niet te veel taken met lagere voorkeur worden toegewezen en we toch een gebalanceerder resultaat krijgen qua tijdsbesteding.

\clearpage

## Een eerste alternatief met een hogere kost voor afwijkingen in tijdsbestedingen
De eerste oplossing toont dat er slechts 4 taken worden toegewezen aan teamleden die deze taak niet in hun top `r gebruikte_top` hadden staat. Het betreft een taak die door 9/10 teamleden MOET uitgevoerd worden. Dit is dus niet zo verwonderlijk. 

In de resultaten hieronder kiezen we $p_t = 10$ in plaats van $p_t = 5$. Hierdoor verwachten we dat de verdeling in tijdsbestedingen eerlijker is maar misschien verliezen we daardoor wel wat op vlak van voorkeuren.


```{r echo=FALSE, warning = FALSE, include = FALSE}
gewicht_afwijking_tijdsinvestering <- 10
model <- MIPModel() %>% 
  add_variable(x[i, j], i = 1:nb_tasks, j = 1:nb_team, type = "binary") %>% 
  add_variable(y[r, j], r = 1:nb_interactie, j = 1:nb_team, type = "binary") %>%
  add_variable(s_min[j], j = 1:nb_team, type = "continuous", lb = 0) %>% 
...
Dit resultaat ziet er al iets beter uit. We lijken niet al te veel te verliezen op vlak van voorkeuren en de tijdsbesteding is gebalanceerder. We gaan na of we dit eventueel nog kunnen verbeteren.

\clearpage

## Een tweede alternatief met een extra beperking
We zien dat teamlid 6 wat minder moet presteren en een teamlid 4 meer dan een eerlijke toewijzing zou voorschrijven. 

De resultaten hieronder beschrijven de resultaten waarbij $s^-_4<1.5$ toegevoegd wordt aan het model om er voor te zorgen dat dit teamlid minder tijd moet besteden aan teamtaken dan in de vorige oplossing. 

```{r echo=FALSE, warning = FALSE, include = FALSE}
result <- model %>% 
  add_constraint(s_min[j] <= 1.5, j = 4) %>%
  solve_model(with_ROI("glpk", verbose = TRUE))
id <- "alternatiefa"
```
...
De resultaten hieronder beschrijven de resultaten waarbij $s^-_4<1.5$ en $s^-_1<1.5$ toegevoegd wordt aan het model. Hoewel de tijdsbesteding nu behoorlijk gebalanceerd is, zien we dat er iemand een heel grote taak krijgt toegewezen die niet meer in zijn/haar top 15 staat. Dit lijkt op deze manier een straatje zonder einde. 

```{r echo=FALSE, warning = FALSE, include = FALSE}
result <- model %>% 
  add_constraint(s_min[j] <= 1.5, j = 4) %>%
  add_constraint(s_min[j] <= 1.5, j = 1) %>%
  solve_model(with_ROI("glpk", verbose = TRUE))
id <- "alternatiefb"
```
...
Na nog enkele extra testen (kosten wijzigen, beperkingen toevoegen,...) is het duidelijke dat er een keuze gemaakt moet worden tussen een licht ongebalanceerde tijdsbesteding (beste lijkt eerste alternatief) of een grote taak aan iemand toewijzen die de taak niet in zijn/haar top 15 had gezet (Derde alternatief). Waar geven we de voorkeur aan? 
